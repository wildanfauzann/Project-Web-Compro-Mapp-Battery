            ...
            <div class="flex flex-col gap-1.5">
                ...
                <div class="flex items-center gap-3 text-[11px] md:text-xs text-[#1f1f1f] font-semibold">
                    ...
                    <div class="relative" id="language-dropdown-container">
                        <button type="button" id="language-dropdown-toggle" class="navbar-top-btn px-1 py-0.5 text-[#1f1f1f] rounded btn-hover flex items-center gap-1.5 hover:bg-[#efefef]">
                            <img src="{{ asset('images/icon/image 26.png') }}" alt="Icon bahasa" class="w-4 h-4 object-contain" />
                ...
                <div class="relative">
                    <input 
                        ...
                        class="navbar-search-input w-full px-3 py-1 pr-7 bg-[#f2cd00] border border-[#e2bd00] rounded-full text-[12px] text-[#222222] placeholder:text-[#404040] focus:outline-none focus:ring-2 focus:ring-[#d4b100] transition-all"
                    />
        ...
        <div class="max-w-7xl mx-auto px-4 md:px-8 relative" id="navbar-preview-area">
            <ul id="navbar-menu" class="navbar-menu-list flex items-center justify-between text-[11px] md:text-sm font-semibold text-[#161616]">
                <li class="relative flex-1 text-center navbar-item {{ Request::is('/') ? 'active' : '' }}" data-preview-key="beranda">
                    ...
                    <a href="{{ url('/') }}" data-section="beranda" class="navbar-link block py-3" @if (Request::is('/')) aria-current="page" @endif>Beranda</a>
                </li>
                <li class="relative flex-1 text-center navbar-item {{ Request::is('tentang*') ? 'active' : '' }}" data-preview-key="tentang">
                    <span class="navbar-top-indicator"></span>
                    <a href="{{ url('/tentang') }}" data-section="tentang" class="navbar-link block py-3" @if (Request::is('tentang*')) aria-current="page" @endif>Tentang Kami</a>
                </li>
                <li class="relative flex-1 text-center navbar-item {{ Request::is('produk*') ? 'active' : '' }}" data-preview-key="produk">
                    <span class="navbar-top-indicator"></span>
                    <a href="{{ url('/produk') }}" data-section="produk" class="navbar-link block py-3" @if (Request::is('produk*')) aria-current="page" @endif>Produk</a>
                </li>
                <li class="relative flex-1 text-center navbar-item {{ Request::is('layanan*') ? 'active' : '' }}" data-preview-key="layanan">
                    <span class="navbar-top-indicator"></span>
                    <a href="{{ url('/layanan') }}" data-section="layanan" class="navbar-link block py-3" @if (Request::is('layanan*')) aria-current="page" @endif>Layanan</a>
                </li>
                <li class="relative flex-1 text-center navbar-item {{ Request::is('unduhan*') ? 'active' : '' }}" data-preview-key="unduhan">
                    <span class="navbar-top-indicator"></span>
                    <a href="{{ url('/unduhan') }}" data-section="unduhan" class="navbar-link block py-3" @if (Request::is('unduhan*')) aria-current="page" @endif>Unduhan</a>
                </li>
            </ul>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-1 grid grid-cols-3 gap-4" id="navbar-preview-list">
                            <article class="navbar-preview-card bg-slate-100 rounded-lg p-3">
                                <img src="/images/product/tractionhawcker.png" alt="Preview produk" class="h-24 w-full rounded-md object-contain bg-white shimmer mb-2" id="navbar-preview-image-1" />
                                <p class="text-xs leading-5 text-slate-700" id="navbar-preview-text-1">
                                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                </p>
                            </article>
                            <article class="navbar-preview-card bg-slate-100 rounded-lg p-3">
                                <img src="/images/product/chargerhigh.png" alt="Preview produk" class="h-24 w-full rounded-md object-contain bg-white shimmer mb-2" id="navbar-preview-image-2" />
                                <p class="text-xs leading-5 text-slate-700" id="navbar-preview-text-2">
                                    Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
                                </p>
                            </article>
                            <article class="navbar-preview-card bg-slate-100 rounded-lg p-3">
                                <img src="/images/product/connector.png" alt="Preview produk" class="h-24 w-full rounded-md object-contain bg-white shimmer mb-2" id="navbar-preview-image-3" />
