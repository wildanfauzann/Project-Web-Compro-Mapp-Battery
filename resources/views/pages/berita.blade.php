                    ...
                    @forelse ($articles as $article)
                        <article class="berita-card group flex flex-col h-full rounded-2xl border border-[#e5edff] bg-white shadow-[0_8px_20px_rgba(15,23,51,0.08)] overflow-hidden transition-all hover:-translate-y-1 hover:shadow-[0_16px_32px_rgba(15,23,51,0.16)]">
                            <div class="relative overflow-hidden bg-[#f0f4ff] h-48 md:h-56">
                                <img src="{{ asset($article['image']) }}" alt="{{ $article['title'] }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" width="640" height="400" loading="lazy" decoding="async" />
                            ...
                            <div class="flex flex-col flex-1 p-5 md:p-6">
                                ...
                                <a href="{{ url('/berita/' . $article['slug']) }}" class="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-[#2f4a99] transition-colors hover:text-[#f2cd00]">
