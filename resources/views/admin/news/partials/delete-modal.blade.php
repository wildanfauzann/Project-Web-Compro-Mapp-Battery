<!-- Delete Modal -->
<div id="delete-modal-{{ $artikel->id }}" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
            ...
            <div class="relative w-full max-w-md transform overflow-hidden rounded-3xl bg-white p-6 text-left shadow-2xl transition-all">
                <div class="flex items-start gap-4">
                    ...
                    <div class="mt-1 sm:mt-0">
                        <h3 class="text-lg font-semibold text-slate-900">Hapus Artikel</h3>
                        <p class="mt-2 text-sm text-slate-500">Anda yakin ingin menghapus artikel <strong>{{ Str::limit($artikel->judul, 30) }}</strong>? Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
                
                <div class="mt-6 flex gap-3 sm:mt-5 sm:justify-end">
                    <button type="button" data-modal-close class="btn-outline">Batal</button>
                    <form method="POST" action="{{ route('admin.berita.destroy', $artikel) }}">
                        @csrf
                        @method('DELETE')
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const trigger = document.querySelector('[data-modal-target="delete-modal-{{ $artikel->id }}"]');
        const modal = document.getElementById('delete-modal-{{ $artikel->id }}');
